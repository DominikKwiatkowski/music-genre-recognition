...
def load_spectrogram_from_path(
        image_path,
        image_label,
        data_w
...
def get_datasets(
    ...
    paths_list, labels_list = get_image_paths_and_labels(training_metadata, training_path, training_config)

    training_dataset = tf.data.Dataset.from_tensor_slices((paths_list, labels_list))
    training_dataset = (
        training_dataset
            .map(lambda path_tensor, label_tensor, data_w=training_config.input_w:
                 tf.numpy_function(load_spectrogram_from_path, [path_tensor, label_tensor, data_w],
                                   (tf.float32, tf.int64)),
    ...
    paths_list, labels_list = get_image_paths_and_labels(validation_metadata, validation_path, training_config)
