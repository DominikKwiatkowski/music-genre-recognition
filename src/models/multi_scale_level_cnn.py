...
def multi_scale_block(num_conv_filters, initializer):
    def f(input_):
        ...
        branchpool = MaxPooling2D(pool_size=(3, 3), strides=(1, 1), padding="same")(
            input_
        )
...
def multi_scale_level_cnn(
    input_shape, num_dense_blocks, num_conv_filters, num_classes, initializer
):
    model_input = Input(shape=input_shape)

    x = Convolution2D(
        num_conv_filters, 3, padding="same", kernel_initializer=initializer
    )(model_input)
